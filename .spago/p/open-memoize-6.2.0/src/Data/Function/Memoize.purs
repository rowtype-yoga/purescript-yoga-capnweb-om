...
instance tabulateTuple :: (Tabulate a, Tabulate b) => Tabulate (Tuple a b) where
  tabulate f =
    let
      f' = tabulate \a -> tabulate \b -> f (Tuple a b)
    in
      \(Tuple a b) -> do
        g <- f' a
        g b
...
instance tabulateProduct :: (Tabulate a, Tabulate b) => Tabulate (Product a b) where
  tabulate f =
    let
      f' = tabulate \a -> tabulate \b -> f (Product a b)
    in
      \(Product a b) -> do
        g <- f' a
        g b
